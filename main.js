...
let intersectionsList = document.querySelector(".intersections"); // -- Нашли список с элементами совпадения
    ...
    xhr.onload = function () {
        ...
        for (let i = 0; i < data.length; i++) {
            if (value != "") {
                ...
                intersectionsList.append(itemElement);
            }
        }

        let searchItems = document.querySelectorAll(".intersections__item");
        searchItems.forEach((item) => {
            item.addEventListener("click", (event) => {
                let textContent = event.target.textContent;
                console.log("textContent: " + textContent)
                downloadData(1, textContent);
...
function downloadData(page = 1, autocompleteValue) {
    ...
    xhr.onload = function () {
        ...
        searchIntersections(this.response.records);
    };
    xhr.send();
...
function foundFact() { //функция для работы поиска, отправляет запрос
    let url = new URL("http://cat-facts-api.std-900.ist.mospolytech.ru/facts");//определяем url-ссылку на все факты
    url.searchParams.append('q', document.getElementById("search-field").value);//параметр, введенный пользователем
    let xhr = new XMLHttpRequest();//определяет http-запрос без перезагрузки
    xhr.open('GET', url);//инициализируем метод запроса и url 
    xhr.responseType = 'json';//определяем тип ответа на запрос - json
    xhr.onload = function () {//вызов функции
        renderRecords(this.response.records);//добавляет записи
        setPaginationInfo(this.response['_pagination']);//определяет, какая 1 запись по счету, какая последняя, и сколько их всего
        renderPaginationElement(this.response['_pagination']);//определяет переходы на другие страницы
    }
...
function autocompletionFact() {//функция для автозаполнения
    ...
    xhr.onload = function () {
        ...
        this.response.forEach(obj => {//для каждого введенного символа выводится список слов, начинающихся с этого символа
            let word = document.createElement('div');//созаем пустой div(элемент, в котором будут выводиться подсказки)
            word.classList.add('autocomplete-item');//метод добавляет элементу класс, указанный в скобках
...
function autocompletionFactEvent(event) {//функция записи выбранной подсказки в поисковую строку
    ...
    searchField.value = event.target.innerHTML;//заполнение поисковой строки выбранной подсказкой
...
window.onload = function () {// вызов функций
    ...
    document.querySelector('.pagination').onclick = pageBtnHandler;
    document.querySelector('.per-page-btn').onchange = perPageBtnHandler;
    document.querySelector('.search-btn').onclick = foundFact;//если нажали "найти"
    document.querySelector('.search-field').oninput = autocompletionFact;//если начали вводить в поисковую строку
    document.querySelector('.autocompletion').onclick = autocompletionFactEvent;//если нажали на подсказку
}